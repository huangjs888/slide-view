...
export const cTransform = function cTransform(confirm, translate = 0) {
  const {
    contentEl,
    leftActions,
    rightActions,
...
export const transform = function transform(translate, duration = this.duration) {
  const {
    leftEl,
    rightEl,
    contentEl,
  ...
  let factor = 0;
  const transition = duration <= 0 ? '' : `transform ${duration}s ${timing} 0s`;
  const wTransition = duration <= 0 ? '' : `width ${duration}s ${timing} 0s, transform ${duration}s ${timing} 0s`;
  const aTransform = ({
    style,
    items,
    element,
    ...
    for (let i = len; i >= 0; i--) {
      ...
      // 左边或右边的最后一个按钮
      setStyle(wrapper, _extends({
        transform: `translate3d(${(i === len && this._overshooting ? translate : transformx) + delta}px, 0, 0)`,
        transition
      }, styleObj));
      // 累计已滑出按钮的占比距离
