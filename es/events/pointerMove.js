...
export default function pointerMove(e) {
  ...
  if (actions && !actions.disable) {
    ...
    const otSize = factor * Math.min(this._width, Math.max(this._width - overshootFreeSize, tWidth));
    ...
    if (overshoot) {
      ...
      if (Math.abs(currentTranslate) >= Math.abs(otSize)) {
        ...
        translate = performDamping(currentTranslate - otSize, {
          expo: friction
        }) + otSize;
      ...
      if (Math.abs(currentTranslate) >= Math.abs(oaSize)) {
        translate = performDamping(currentTranslate - oaSize, {
          expo: friction
        }) + oaSize;
      } else {
        translate = currentTranslate;
