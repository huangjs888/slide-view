...
export default function buttonPress(event, direction) {
  const {
    element,
    leftActions,
    rightActions,
  ...
  const target = findTarget(sourceEvent, t => t !== currentTarget && !t.getAttribute('data-index'));
      ...
      if (item.confirm) {
        ...
        if (rebounce > 0 && index !== 0 /*  &&
          ...
          onOnceTransitionEnd(item.wrapper, () => {
            ...
            if (this._confirming && this._confirming.index === confirm.index && this._confirming.direction === confirm.direction) {
