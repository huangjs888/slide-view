 ...
 * @LastEditors: Huangjs
 * @LastEditTime: 2023-02-23 14:40:14
 * @Description: ******
 */
...
const move = function move(this: SlideView, e: GestureEvent) {
  ...
  if (slidePage < slideMin) {
    // 最后一个按钮滑动覆盖所有按钮并执行操作
    movex = (this.rebounce ? rebounceSize(slidePage - slideMin) : 0) + slideMin;
    if (slidePage < slideMin - slideSize) {
      isDel = true;
    }
    ...
    movex = 0;
    // movex = (this.rebounce ? rebounceSize(a-slideMax) : 0)+slideMax;
  }
  ...
  for (let i = this.buttons.length - 1; i >= 0; i--) {
    ...
    setStyle(this.buttons[i].element, {
      transform: `translateX(${isDel ? movex : transformx}px)`,
      transition: '',
    });
    // 累计已滑出按钮的占比距离
