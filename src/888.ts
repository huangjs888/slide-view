 ...
 * @LastEditors: Huangjs
 * @LastEditTime: 2023-04-04 11:51:54
 * @Description: ******
 */
...
const move = function move(this: SlideView, e: AgentEvent) {
  ...
  if (actions && !actions.disable) {
    ...
    const otSize =
      vector * Math.min(elWidth, Math.max(elWidth - overshootFreeSize, tWidth));
    const oeSize =
      vector * Math.min(elWidth * 0.5, Math.max(0, overshootEdgeSize));
  ...
  if (
    ...
    if (item.confirm) {
      ...
      if (actions.items.length === 1) {
        translate =
          (Math.min(Math.abs(2 * this._translate), elWidth) * translate) /
          Math.abs(translate);
      }
      ...
      if (rebounce > 0 && actions.style !== 'round' && index !== 0) {
        onOnceTransitionEnd(item.wrapperEl, () => {
...
const buttonSlide = function buttonSlide(
  ...
  const actions =
    direction === 'left'
      ...
      ? rightActions
      : null;
  if (this._translate === 0 || !actions || actions.disable || !element) {
    return;
  }
  const elWidth = element.getBoundingClientRect().width;
  const item = actions.items[index];
  ...
  if (
    ...
    // 无论是否确认都需要overshoot
    const translate = (this._translate * elWidth) / Math.abs(this._translate);
  ...
  setActions(actions: IActionOption = {}, direction: Direction = 'both') {
    ...
    const _setActions = (_direction: 'left' | 'right') => {
      ...
      if (actions.items && actions.items.length > 0) {
        ...
        addClass(parentEl, `hjs-slideview__${style} ${className || ''}`);
        let tWidth = 0;
        const nItems = items.map((item, index) => {
          ...
          // 设置非确认时的样式和内容
          confirmStyle(style, mItem);
          const margin = getMarginSize(element);
...
type MergeActionItem = {
  wrapperEl: HTMLElement; //当前按钮包装元素
...
type MergeAction = {
  ...
  overshootFreeSize: number; // overshoot时超过这尺寸，最后一个按钮直接滑动到这个尺寸
  overshootEdgeSize: number; // 手指滑动到接近屏幕边缘尺寸（小于这个尺寸就overshoot）
  width: number; // 所有按钮展开情况下总宽度
