 ...
 * @LastEditors: Huangjs
 * @LastEditTime: 2023-03-31 13:38:37
 * @Description: ******
 */
...
const cTransform = function cTransform(
  ...
  const aTransform = ({ items }: MergeAction) => {
    // 前面已有按钮的占比距离
    let translateDetlaTotal = 0;
    for (let i = items.length - 1; i >= 0; i--) {
      const { wrapper, width } = items[i];
      if (items.length === 1) {
      ...
      if (i === index) {
        ...
        if (translate !== 0) {
          ...
          translateX = (width + translateDetlaTotal) * factor;
        }
        setStyle(wrapper, {
...
const transform = function transform(
  ...
  const translateMax = (translate * _elWidth) / Math.abs(translate);
  const transition = duration <= 0 ? '' : `transform ${duration}s ${timing} 0s`;
  const aTransform = (
    { items, width: tWidth, gap: _tGap }: MergeAction,
    x: number,
  ) => {
    // 前面已有按钮的占比距离
    let translateDetlaTotal = 0;
    const len = items.length - 1;
    const tGap = (x * _tGap) / Math.abs(x);
    const gap = tGap / len;
    for (let i = len; i >= 0; i--) {
      const { wrapper, inner, width } = items[i];
      // 当前按钮需要滑出的占比距离
      const translateDetla = (width / tWidth) * (x - tGap);
      // 累计已滑出按钮的占比距离
      translateDetlaTotal += translateDetla;
      setStyle(wrapper, {
        transform: `translate3d(${
          translateMax -
          translateDetla +
      ...
      setStyle(inner, {
        transform: `translate3d(${translateDetla - translateMax}px, 0, 0)`,
        transition,
      });
    }
  };
  ...
  window.requestAnimationFrame(() => {
    ...
    if (leftActions) {
      ...
      aTransform(leftActions, translate);
    }
    if (rightActions) {
      setStyle(rightEl, {
        transform: `translate3d(${translate}px, 0, 0)`,
      ...
      const { width, gap, style } = rightActions;
      if (style === 'drawer') {
        ...
        aTransform(rightActions, Math.min(-(width + gap), translate));
      } else {
        aTransform(rightActions, translate);
      }
    }
  });
};
    ...
    if (typeof threshold === 'number') {
      ...
      if (
        ...
        this.rightActions.threshold = Math.min(
          _threshold,
          this.rightActions.width + this.leftActions.gap,
        );
      }
    }
  }
  setActions(actions: IActionOption = {}, direction: Direction = 'both') {
    ...
    const _setActions = (_direction: 'left' | 'right') => {
      ...
      if (actions.items && actions.items.length > 0) {
        ...
        const _tWidth = Math.min(
          tWidth,
          Math.max(this._elWidth - overallFreeSize, 0),
        );
        const gap = 20;
        const _tGap = gap * (newItems.length - 1);
        let _sWidth = 0;
        newItems = newItems.map((item, i) => {
          const { wrapper, inner, element, width, margin, background } = item;
          const _nWidth = (_tWidth * width) / tWidth;
          setStyle(element, { width: _nWidth - margin });
          setStyle(inner, {
            background: (style !== 'round' ? background : '') || 'inherit',
          });
          return {
            ...item,
            width: _nWidth,
        ...
        this[`${_direction}Actions`] = {
          ...
          overshootSize: Math.min(
            this._elWidth,
            Math.max(this._elWidth - overshootFreeSize, _tWidth),
          ),
          overshootEdgeSize: Math.min(
            this._elWidth * 0.5,
            Math.max(0, overshootEdgeSize),
          ),
          threshold: Math.min(_tWidth, Math.max(threshold, 0)),
          element: actionEl,
          items: newItems,
        };
      }
...
export type ActionStyle = 'round' | 'rect' | 'drawer';
...
type MergeActionItem = {
  wrapper: HTMLElement; //当前按钮包装元素
  inner: HTMLElement; //当前按钮内部元素
...
type MergeAction = {
  ...
  element: HTMLElement; //当前所有按钮的父元素
  width: number; // 当前所有按钮的总尺寸
