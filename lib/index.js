...
var start = function start(e) {
  ...
  if (actions && !actions.disable) {
    ...
    if (Math.abs(this._translate) <= Math.abs(criticalTranslate)) {
      ...
      // 恢复_translate的弹性尺寸部分
      startTranslate = (0, _util.rebounceSize)(this._translate - criticalTranslate, friction, true) + criticalTranslate;
    }
  }
...
var move = function move(e) {
  ...
  if (actions && !actions.disable) {
    ...
    var otSize = vector * Math.min(this._width, Math.max(this._width - overshootFreeSize, tWidth));
    var oeSize = vector * Math.min(this._width * 0.5, Math.max(0, overshootEdgeSize));
    var oaSize = vector * tWidth;
    // 可以overshoot的情况处理
    if (overshoot) {
      ...
      if (Math.abs(currentTranslate) >= Math.abs(otSize)) {
        ...
        // 进行overshoot滑动
        translate = (0, _util.rebounceSize)(currentTranslate - otSize, friction) + otSize;
      ...
      if (Math.abs(currentTranslate) >= Math.abs(oaSize)) {
        translate = (0, _util.rebounceSize)(currentTranslate - oaSize, friction) + oaSize;
      } else {
        translate = currentTranslate;
      }
    }
...
var end = function end(e) {
  var element = this.element,
    leftActions = this.leftActions,
    rightActions = this.rightActions;
  if (!this._isMoving || !this._startPoint || this._startAngle !== 1 || !element || (!leftActions || leftActions.disable) && (!rightActions || rightActions.disable)) {
    return;
  }
  ...
  if (actions && !actions.disable) {
    ...
    if (this._overshooting) {
      var index = actions.items.length - 1;
      var item = actions.items[index];
      var translate = this._translate * element.getBoundingClientRect().width / Math.abs(this._translate);
      this._translate = translate;
    ...
    value: function show() {
      ...
      return new Promise(function (resolve) {
        var contentEl = _this7.contentEl,
          rebounce = _this7.rebounce,
          leftActions = _this7.leftActions,
          rightActions = _this7.rightActions;
        if (_this7._destory || !contentEl || (!leftActions || leftActions.disable) && (!rightActions || rightActions.disable)) {
          resolve();
          return;
        }
    ...
    value: function hide() {
      ...
      return new Promise(function (resolve) {
        var contentEl = _this8.contentEl,
          leftActions = _this8.leftActions,
          rightActions = _this8.rightActions;
        if (_this8._destory || _this8._translate === 0 || !contentEl || (!leftActions || leftActions.disable) && (!rightActions || rightActions.disable)) {
          resolve();
          return;
        }
        _this8._translate = 0;
